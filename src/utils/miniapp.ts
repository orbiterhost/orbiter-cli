	...
	try {
		...
		try {
			...
			try {
				...
				if (!siteReq.ok) {
					throw new Error(
						`Problem retrieving site data: ${JSON.stringify(siteData)}`,
					);
				}
			...
			const farcasterConfigPath = path.join(
				targetDir,
				"public/.well-known/farcaster.json",
			);
			if (fs.existsSync(farcasterConfigPath)) {
				try {
					await setupFarcasterAccountAssociation(
						deployedSiteId,
						farcasterConfigPath,
					);
			...
			spinner.succeed(
				`🚀 Mini App deployed successfully to https://${domain}.orbiter.website`,
			);
		} finally {
...
function processIndexHtml(content: string, options: TemplateOptions): string {
	const domain = options.domain
		? `${options.domain}.orbiter.website`
		: "your-app.orbiter.website";
	...
	try {
		...
		if (!siteId) {
			const error = new Error(
				"Cannot setup account association: Site ID is missing or undefined",
			);
			throw error;
		}
		...
		if (!fs.existsSync(configPath)) {
			const error = new Error(
				`Cannot setup account association: Config file not found at ${configPath}`,
			);
			throw error;
		}
		...
		if (!tokens) {
			const error = new Error(
				"Cannot setup account association: Authorization required. Please login first.",
			);
			throw error;
		}

		try {
			const accountAssociationReq = await fetch(
				`${API_URL}/farcaster/account_association/${siteId}`,
				{
					method: "POST",
					headers: {
						Source: `${SOURCE}`,
			...
			if (!accountAssociationReq.ok) {
				...
				const error = new Error(
					`Account association API call failed. ${errorDetail}`,
				);
				throw error;
			}
			...
			try {
				...
				if (!associationData) {
					const error = new Error(
						"API returned empty response for account association",
					);
					throw error;
				}
			} catch (jsonError: any) {
			...
			try {
				...
				const error = new Error(
					`Failed to read or parse farcaster.json: ${readError.message}`,
				);
				throw error;
			}
			...
			if (
				associationData &&
				typeof associationData === "object" &&
				"accountAssociation" in associationData
			) {
			...
			try {
				...
				const error = new Error(
					`Failed to write updated farcaster.json: ${writeError.message} `,
				);
				throw error;
			}
			...
			throw new Error(
				`Failed to set up account association with API: ${apiError.message} `,
			);
		}
	} catch (error: any) {
